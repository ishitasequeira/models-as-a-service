...
func TestFilterByStatus(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		...
		err := store.AddKey(ctx, "testuser", keyID, keyHash, name, "", []string{"system:authenticated"}, nil, false)
		require.NoError(t, err)
	}
...
func TestFilterByMultipleStatuses(t *testing.T) {
	...
	err := store.AddKey(ctx, "testuser", keyID, keyHash, "Revoked Key", "", []string{"system:authenticated"}, nil, false)
	require.NoError(t, err)
	err = store.Revoke(ctx, keyID)
	require.NoError(t, err)
...
func TestFilterByUsernameAndStatus(t *testing.T) {
	...
	err := store.AddKey(ctx, "alice", keyID, keyHash, "Alice Revoked Key", "", []string{"system:authenticated"}, nil, false)
	require.NoError(t, err)
	err = store.Revoke(ctx, keyID)
	require.NoError(t, err)
	...
	err = store.AddKey(ctx, "bob", keyID, keyHash, "Bob Active Key", "", []string{"system:authenticated"}, nil, false)
	require.NoError(t, err)
