...
setup_admin_token() {
  ...
  if [[ -z "${admin_bootstrap_token}" ]]; then
    echo "[smoke] Failed to get admin bootstrap token (cert-based auth?) - admin tests will be skipped"
    return 0
  fi
  
  # Mint admin API key using admin's bootstrap token
  echo "[smoke] Minting admin API key..."
  local admin_api_key
  admin_api_key=$(mint_api_key "e2e-admin-${current_user}" "${admin_bootstrap_token}")
